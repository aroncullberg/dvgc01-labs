...
convert_token(Token, Code) :- Token \= integer, Token \= program, Token \= begin, Token \= input, Token \= output, Token \= var, Token \= end, atom_chars(Token, [First | _]), char_type(First, alpha), atom(Token), token_value(id, Code).
